  ...
  <div>
    <form class="w-full add-role" @submit.prevent="handleSubmit">
      <div class="container">
        ...
        >
          <page-heading title="تعديل مشروع " desc="قم بتعديل بيانات المشروع" />
    ...
    <!-- Success Modal -->
    <success-modal v-if="showModel"> تم تعديل مقالة جديد بنجاح </success-modal>
  </div>
</template>
  ...
  methods: {
    async handleShow() {
      const res = await this.$axios.get(
        `dashboard/pages/projects/${this.$route.params.id}`
      );
      const { data } = res.data;
      const { is_active, image, translations } = data;
