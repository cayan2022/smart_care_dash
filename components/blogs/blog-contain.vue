  ...
  <div>
    <section class="blogs">
      <div class="container">
        ...
        >
          <page-heading title="المدونة" desc="جميع المدونات الخاصة بالنظام" />
          ...
          <div class="flex items-center content-center bottom-action">
            ...
            <page-button
              v-if="hasPermissions(`create_blogs`)"
              name="مقالة جديدة"
              @click.prevent="$router.push(localePath(`/blogs/add`))"
              btnClass="yellow"
              icon="add-square"
            />
          </div>
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                <li class="w-full py-3" v-if="hasPermissions(`update_blogs`)">
                  <nuxt-link
                ...
                <li
                  ...
                  v-if="
                    hasPermissions(
                      `${blog.is_block ? 'active' : 'block'}_blogs`
                    )
                  "
                >
                  <!-- <button
                  ...
                  >
                    <img
                      :src="`/img/icons/${
                        blog.is_block ? 'shield_tick' : 'slash'
                      }.svg`"
                      class="w-5 h-5 object-contain ltr:mr-2 rtl:ml-2"
                      loading="lazy"
                      :alt="$t(`app.title`)"
                    ...
                    <span class="text-dark text-sm font-medium">
                      {{ `${!blog.is_block ? "تعطيل" : "تفعيل"}` }}
                    </span>
                  </button>
                </li>

                <li class="w-full py-3" v-if="hasPermissions(`delete_blogs`)">
                  <!-- <button
                    class="data-contain flex items-center content-center"
    ...
    >
      ...
      >
        <img
          :src="`/img/form/${blog.is_block ? 'done' : 'block'}.svg`"
          class="w-16 h-16 object-contain"
          loading="lazy"
          :alt="$t(`app.title`)"
        />
...
export default {
  ...
  methods: {
    handleDeleteBlog() {
      this.$axios
        .$post(`dashboard/pages/blogs/${this.blog.id}`, {
          _method: "delete",
        })
        .then(() => {
    ...
    handleBlogStatus() {
      const blogStatus = this.blog.is_block ? "active" : "block";
      this.$axios
        .$post(`dashboard/pages/blogs/${this.blog.id}/${blogStatus}`, {
          _method: "put",
        })
        .then(() => {
