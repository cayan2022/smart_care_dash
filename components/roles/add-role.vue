  ...
  <div>
    <form class="w-full add-role" @submit.prevent="handleSubmit">
      <div class="container">
        ...
        >
          <page-heading
            title="انشاء صلاحية جديدة"
            desc="قم بتعبئة البيانات الآتية لانشاء صلاحية جديدة"
          />
          ...
          <div class="flex items-center bottom-action">
            ...
            <page-button
              name="الغاء"
              @click.prevent="$router.push(localePath(`/roles`))"
              btnClass="black"
              imgClass="hidden"
            />
          </div>
...
export default {
  ...
  methods: {
    handleSubmit() {
      this.loading = true;
      this.$axios
        .$post("dashboard/roles", this.form)
        .then(() => {
