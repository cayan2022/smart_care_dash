  ...
  <div>
    <section class="services">
      <div class="container">
        ...
        >
          <page-heading title="الخدمات" desc="خدمات النظام التي يقدمها لك" />
          ...
          <div class="flex items-center content-center bottom-action">
            ...
            <page-button
              v-if="hasPermissions('create_services')"
              name="خدمة جديدة"
          ...
          >
            ...
            >
              ...
              <p class="text-gray text-base font-normal">
                {{ service.description | maxWord(10) }}
              </p>
            </div>

            <div
              @click="service.dropList = !service.dropList"
              :class="service.dropList === true ? 'activeList' : ''"
              class="box-drop-list w-fit absolute top-5 z-10 ltr:right-4 cursor-pointer rtl:left-4"
            >
              <img
              ...
              >
                <li
                  class="w-full py-3"
                  v-if="hasPermissions('update_services')"
                >
                  <nuxt-link
                    :to="localePath(`/services/${service.id}/edit`)"
                ...
                <li
                  ...
                  v-if="
                    hasPermissions(
                      `${service.is_block ? 'active' : 'block'}_services`
                    )
                  "
                >
                  <button
                    class="data-contain flex items-center content-center"
                    @click="OpenModalPlusId('modalBlock', service)"
                  >
                    <img
                      :src="`/img/icons/${
                        service.is_block ? 'shield_tick' : 'slash'
                      }.svg`"
                      class="w-5 h-5 object-contain ltr:mr-2 rtl:ml-2"
                      loading="lazy"
                      :alt="$t(`app.title`)"
                ...
                <li
                  class="w-full py-3"
                  v-if="hasPermissions('delete_services')"
                >
                  <button
                    class="data-contain flex items-center content-center"
                    @click="OpenModalPlusId('modalDelete', service)"
                  >
                    <img
    ...
    >
      ...
      >
        <img
          :src="`/img/form/${service.is_block ? 'done' : 'block'}.svg`"
          class="w-16 h-16 object-contain"
          loading="lazy"
          :alt="$t(`app.title`)"
        />
        ...
        <p class="text-dark font-bold text-center text-base my-5">
          <template v-if="!service.is_block">
            هل أنت متأكد من تعطيل الخدمة ؟
          </template>
          <template v-else> هل أنت متأكد من تفعيل الخدمة ؟ </template>
        </p>
...
export default {
  ...
  methods: {
    handleDeleteService() {
      this.$axios
        .$post(`dashboard/pages/services/${this.service.id}`, {
          _method: "delete",
        })
        .then(() => {
    ...
    handleServiceStatus() {
      const serviceStatus = this.service.is_block ? "active" : "block";
      this.$axios
        .$post(`dashboard/pages/services/${this.service.id}/${serviceStatus}`, {
          _method: "put",
        })
        .then(() => {
