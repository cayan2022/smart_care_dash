  ...
  <div>
    <form class="w-full add-role" @submit.prevent="handleSubmit">
      <div class="container">
        ...
        >
          <page-heading title="اضافة خبر " desc="قم باضافة خبر جديد" />
    ...
    <!-- Success Modal -->
    <success-modal v-if="showModel"> تم انشاء خبر جديد بنجاح </success-modal>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async handleSubmit() {
      try {
        this.loading = true;
        await this.$axios.post(`dashboard/pages/tidings`, this.generateData());
        this.showModel = true;
        this.loading = false;
